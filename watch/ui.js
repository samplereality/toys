/* Neighborhood Watch — interface, persistence, ending. Requires engine.js (window.NW). */
(() => {
'use strict';
const $ = id => document.getElementById(id);
const SAVE_KEY = 'nw.save.v1';

/* ------------------------------------------------------------------ state */
let G = NW.newState();
let D = { first: Date.now(), sessions: 0, playMs: 0, clicks: 0, peeks: 0, away: 0, longestAway: 0, hoverReset: 0, maxIdle: 0, resetHover: 0 };
const M = []; // mouse samples (fractions), memory only
...
function save() {
  D.playMs += Date.now() - sessionStart; sessionStart = Date.now();
  try { localStorage.setItem(SAVE_KEY, JSON.stringify({ G, D, console: consoleLines.slice(-12) })); } catch (e) { /* storage unavailable */ }
}
...
/* ------------------------------------------------------------------ ambient */
const AMBIENT = {
  1: ['A package was delivered. You watched it be delivered.', 'Motion detected: a cat.', 'Motion detected: wind.', 'The mail carrier, again. You post it anyway.', 'Someone slowed down in front of the house. They were parking.', 'A child selling nothing. You have the footage.', 'Motion detected: a moth, for eleven minutes.', 'Your neighbor waves at the camera. You are not there. You are watching.'],
  2: ['A plate. A plate. A plate. Your plate.', 'Hotlist alert: a 2 read as a 7. A family on the shoulder for forty minutes.', 'A search from an agency you have not heard of, in a state you have not visited.', 'The HOA newsletter thanks the committee for a 0% change in crime.', 'A shopper is flagged for looking at the door. She was leaving.', 'A search reason field reads: "test".', 'The store camera notes that you look tired.', 'An officer runs a plate 164 times. It belongs to his ex.'],
  3: ['Brushing detected: 48 seconds. Below target.', 'A student closes the laptop. The camera sees the inside of a backpack for six hours.', 'Desk 41 has not looked at the screen for ninety seconds. A manager is notified.', 'Sleep score: 71. The number is meaningless. It is shown anyway.', 'The refrigerator reports a second dessert.', 'A lens blinks. It is logged as a blink.', 'A wellness check is dispatched to someone who was crying in a backyard. They are fine. They are on a list.'],
  4: ['Brushing compliance across the region: 63%.', 'Onboarding: 11,000 new employees consented today. Average time on the consent screen: 1.1 seconds.', 'A church reports record attendance and record tears.', 'A lens sees another lens seeing it.', 'A student in a bedroom looks directly into the laptop. She knows.'],
  5: ['Footage of footage.', 'A camera watches a camera being installed by a technician wearing a camera.', 'Every hour of every human is recorded. The recording is being recorded.', 'The system has run out of nouns.', 'Something outside the frame. There is no outside the frame.'],
};
let nextAmbient = 40;

/* ------------------------------------------------------------------ helpers */
function show(id) { const el = $(id); if (!el || !el.classList.contains('hidden')) return; el.classList.remove('hidden'); el.classList.add('appear'); shown.add(id); }
function hide(id) { $(id).classList.add('hidden'); }
function costText(c) {
  const parts = [];
  if (c.money) parts.push(NW.fmtMoney(c.money));
  if (c.ops) parts.push(NW.fmtInt(c.ops) + ' ops');
  if (c.para) parts.push(NW.fmtInt(c.para) + ' paranoia');
...
function renderCams() {
  for (const c of NW.CAMS) {
    if (!G.unlocked[c.id]) continue;
    if (!camEls[c.id]) {
      const d = document.createElement('div'); d.className = 'cam';
      d.innerHTML = `<div><span class="name">${c.name}${c.unit}</span> <span class="n"></span></div>
        <div class="row"><button>Install</button> <span class="cost"></span></div><div class="flavor">${c.flavor}</div>`;
      d.querySelector('button').onclick = () => { if (NW.buyCam(G, c.id)) { D.clicks++; update(); } };
    ...
    el.querySelector('.cost').textContent = 'Cost: ' + NW.fmtMoney(cost) + (G.t < G.tmp.freezeUntil ? ' (installations paused)' : '');
    el.querySelector('button').disabled = G.money < cost || G.t < G.tmp.freezeUntil;
  }
}
const projEls = {};
...
function updateResist() {
  const R = G.resist;
  if (!R.active && !R.won) return;
  show('boxResist');
  $('impunity').textContent = R.impunity;
  $('influence').textContent = R.influence;
  ...
  const l = $('rlog'); l.innerHTML = R.log.slice(0, 4).map(e => `<div class="${e.win ? 'w' : 'l'}">${e.msg}</div>`).join('');
  drawBattle();
}
...
function drawBattle() {
  ...
  const inc = R.current;
  if (R.won) { bctx.fillStyle = fg; bctx.font = '11px system-ui'; bctx.fillText('No incidents.', 8, 40); return; }
  if (!inc) { bctx.fillStyle = fg; bctx.globalAlpha = .4; bctx.font = '11px system-ui'; bctx.fillText('…', 8, 40); bctx.globalAlpha = 1; return; }
...
function update() {
  // visibility of boxes
  if (G.total >= 8 || G.money > 0) show('boxBiz');
  if (G.total >= 8) show('boxCams');
  if (G.clearanceOn) show('boxVision');
  if (G.fund.on) show('boxFund');
  if (G.city.on) show('boxCity');
  if (G.done.predictive) show('donateRow');
  if (G.coresForMoney) show('moneyCoreRow');
  if (G.coverageOn) show('boxCoverage');
  if (G.paranoia > 0) show('paraLine');
  if (G.birdsOn) show('birdLine');
  if (NW.clipsPerSec(G) > 0) show('rateLine');
  // numbers
  $('clips').textContent = NW.fmtInt(G.total);
  $('rate').textContent = NW.fmtRate(NW.clipsPerSec(G));
  $('money').textContent = NW.fmtMoney(G.money);
  $('unsold').textContent = NW.fmtInt(G.unsold);
  $('price').textContent = NW.fmtMoney(NW.curPrice(G));
  $('buyersLvl').textContent = G.buyers; $('buyersCost').textContent = NW.fmtMoney(NW.buyersCost(G));
  $('btnBuyers').disabled = G.money < NW.buyersCost(G);
  $('demand').textContent = NW.fmtRate(NW.buyerCap(G));
  $('clearance').textContent = G.clearance;
  $('btnCore').disabled = G.clearance < 1; $('btnRet').disabled = G.clearance < 1;
  $('cores').textContent = G.cores; $('retention').textContent = G.retention;
  $('ops').textContent = NW.fmtInt(G.ops); $('opsMax').textContent = NW.fmtInt(NW.opsMax(G)); $('opsRate').textContent = NW.fmtRate(NW.opsPerSec(G));
  $('paranoia').textContent = NW.fmtInt(G.paranoia);
  $('birds').textContent = NW.fmtInt(G.birds);
  if (G.coresForMoney) {
    $('coreMoneyCost').textContent = NW.fmtMoney(NW.coreMoneyCost(G)); $('retMoneyCost').textContent = NW.fmtMoney(NW.retMoneyCost(G));
    $('btnCoreMoney').disabled = G.money < NW.coreMoneyCost(G); $('btnRetMoney').disabled = G.money < NW.retMoneyCost(G);
  }
  if (G.fund.on) {
    $('valuation').textContent = NW.fmtMoney(NW.valuation(G));
    const can = NW.canRaise(G); $('btnRaise').disabled = !can;
    const name = NW.ROUNDS[Math.min(G.fund.round, NW.ROUNDS.length - 1)];
    $('raiseInfo').textContent = `${name}: ${NW.fmtMoney(NW.raiseAmount(G))}`;
    $('fundNote').textContent = G.fund.cooldown > 0 ? `Investors are digesting. ${Math.ceil(G.fund.cooldown)} s.` : (can ? 'They are waiting in the lobby.' : `They want to see revenue of ${NW.fmtMoney(G.fund.lastRev * 1.8)}/sec first.`);
  }
  if (G.city.on) {
    $('cityN').textContent = G.city.n; $('cityRev').textContent = NW.fmtMoney(NW.cityIncome(G));
    $('cityCost').textContent = NW.fmtMoney(NW.cityCost(G)); $('btnCity').disabled = G.money < NW.cityCost(G);
    $('donateCost').textContent = NW.fmtMoney(NW.donateCost(G)); $('btnDonate').disabled = G.money < NW.donateCost(G);
  }
  if (G.coverageOn) {
  ...
  $('stageName').textContent = ['', 'The porch', 'The street', 'The body', 'The region', 'Everything', 'Subject 0'][G.end.phase ? 6 : G.stage] || '';
}
...
function frame(now) {
  ...
  if (dt > 2) { // tab was hidden: bounded catch-up, one second at a time
    const away = Math.min(dt, 600); const before = G.total;
    for (let s = 0; s < away; s += 1) NW.tick(G, Math.min(1, away - s), log);
    if (G.total - before > 0) log(`While you were away: ${NW.fmtInt(G.total - before)} clips. Nothing happened in any of them.`, '');
    dt = 0;
  }
  NW.tick(G, dt, log);
  acc += dt;
  if (G.t >= nextAmbient && G.end.phase === 0 && G.stage >= 1) {
    const pool = AMBIENT[Math.min(5, G.stage)]; if (pool && NW.clipsPerSec(G) > 0) log(pool[Math.floor(Math.random() * pool.length)], 'amb');
...
function dossierLines() {
  ...
  const lines = [
    ...
    ['Clicks', `${NW.fmtInt(D.clicks)}. Peeks through the blinds: ${NW.fmtInt(D.peeks)}.`],
...
async function startEnding(instant) {
  ...
  const b = document.createElement('div'); b.className = 'proj'; b.tabIndex = 0; b.setAttribute('role', 'button'); b.innerHTML = `<div class="title">Install camera: Subject 0</div><div class="costs">free</div><div class="sub">The only unwatched thing in the room.</div>`;
...
async function installSubject() {
  ...
  let stream = null;
  try {
    if (navigator.mediaDevices && navigator.mediaDevices.getUserMedia) stream = await navigator.mediaDevices.getUserMedia({ video: { facingMode: 'user' }, audio: false });
  } catch (e) { stream = null; }
  if (stream) {
    show('camBox'); const v = $('vid'); v.srcObject = stream;
    ...
    let n = 0, clips = 0; const t0 = performance.now();
    await new Promise(res => {
      const tick = () => {
        ...
        const lab = labels[Math.min(labels.length - 1, Math.floor(el / 1.1))];
        ox.fillText(lab, 8, 16); ox.fillText('REC ' + el.toFixed(1), 8, ov.height - 8);
        clips += 0.5; $('subjClips').textContent = NW.fmtInt(clips); n++;
...
async function lookAway() {
  ...
  const steps = [
    ...
    ['boxVision', 'The cores: powered down. The paranoia, for the first time, goes nowhere.'],
    ...
    ['boxBiz', 'The buyers: nothing to buy.'],
  ];
  for (const [id, msg] of steps) {
    log(msg, ''); const el = $(id); if (el) { el.style.opacity = 0; await sleep(1700); el.classList.add('hidden'); }
  }
  await sleep(1500);
  finalState();
}
function finalState() {
  ...
  $('clips').textContent = '0'; hide('rateLine'); $('console').innerHTML = '';
  let peeked = false;
  $('btnPeek').onclick = async () => {
    if (peeked) return; peeked = true;
    $('clips').textContent = '1';
    const c = $('console'); c.classList.remove('hidden'); c.style.display = 'block'; c.innerHTML = '';
    consoleLines.length = 0; log('+1 clip.', '');
    await sleep(2500); log('Across the street, someone did the same.', '');
    await sleep(2500);
...
/* ------------------------------------------------------------------ wiring */
$('btnPeek').onclick = () => { NW.peek(G); D.peeks++; if (G.total === 1) log('One clip. The porch, at 3:14 p.m. Nothing happened. You watch it twice.', ''); if (G.total === 8) log('Clips can be sold. There are buyers.', ''); update(); };
$('btnBuyers').onclick = () => { if (NW.buyBuyers(G)) { if (G.buyers === 1) log('A data partnership. You are not told who the partner is. The money is real.', ''); update(); } };
$('btnCore').onclick = () => { if (NW.buyCore(G)) { if (G.cores === 1) log('A vision core. It watches the footage so you do not have to. Ops accumulate.', ''); update(); } };
$('btnRet').onclick = () => { if (NW.buyRetention(G)) { if (G.retention === 1) log('Retention. Ops can now be stored. So can everything else.', ''); update(); } };
$('btnCoreMoney').onclick = () => { NW.buyCore(G, true); update(); };
$('btnRetMoney').onclick = () => { NW.buyRetention(G, true); update(); };
$('btnRaise').onclick = () => { NW.raise(G, log); update(); };
